...
int pass_1()
{
	...
	FILE *f1; //file pointer to read from file
				...
				if(start_error==2)
				{
					...
					if(strcmp(optab_entry.type,"IS")==0)
					{
						...
						if(sym_or_lit == 'S' && sym_entry2.address!=-1)
							printf("%d (IS %d) (R %d) (%c,%d)\n",lc,optab_entry.opcode,reg_entry.num,sym_or_lit,sym_entry2.sr_no);	
						else if(sym_or_lit == 'L')
							printf("%d (IS %d) (R %d) (%c,%d)\n",lc,optab_entry.opcode,reg_entry.num,sym_or_lit,lit_entry.sr_no);
						else
						{
			...
			else if(count == 3 && (strcmp(buff_2,"DS")!=0 && strcmp(buff_2,"DC")!=0 && strcmp(buff_2,"EQU")!=0))
			{
				if(start_error == 2)
				{
					...
					else
					{
						...
						if(sym_or_lit == 'S')
							printf("%d (IS %d) (R %d) (%c,%d)\n",lc,mot.opcode,reg_entry.num,sym_or_lit,sym_entry2.sr_no);	
						else if(sym_or_lit == 'L')
							printf("%d (IS %d) (R %d) (%c,%d)\n",lc,mot.opcode,reg_entry.num,sym_or_lit,lit_entry.sr_no);
					}
					lc++;
				}
				else
...
A      DS     3             100 (DL 1) (C,3)
L1     MOVER  AREG   ='8'   103 (IS 4) (R 0) (L,1)
ADD    AREG   C             104 (IS 1) (R 0) (S,3)
MOVEM  AREG   D             105 (IS 5) (R 0) (S,4)
D      EQU    A+1           106 (AD 5) (S,101)
L2     PRINT  D             107 (IS 10) (S 4)
ORIGIN A-1                  108 (AD 4) (S,99)
C      DC     '5'           99 (DL 2) (C,'5')
ORIGIN L2+1                 100 (AD 4) (S,108)
STOP                        108 (IS 0)
B      DC     '19'          109 (DL 2) (C,'19')
END                         110 (AD 2)
...
L1     MOVER  AREG   ='8'   103 (IS 4) (R 0) (L,1)
ADD    AREG   C             104 (IS 1) (R 0) (S,3)
MOVEM  AREG   D             105 (IS 5) (R 0) (S,1)
D      EQU    A+1           Error! Operand does not have address!
L2     PRINT  D             107 (IS 10) (S 1)
ORIGIN A-1                  Error! Operand does not have address!
C      DC     '5'           -2 (DL 2) (C,'5')
ORIGIN L2+1                 -1 (AD 4) (S,108)
STOP                        108 (IS 0)
B      DC     '19'          109 (DL 2) (C,'19')
END                         110 (AD 2)
